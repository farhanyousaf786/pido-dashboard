/**
 * Shared contact resolution + eligibility for bulk email / SMS audiences.
 */
...
function isGeneratedPhoneEmail(email, phone) {
...
async function resolveUserEmail(admin, uid, userData, profileData) {
...
function resolveUserPhone(userData, profileData) {
  const profile = profileData || {};
  return String(
...
function resolveUserName(userData, profileData) {
  const profile = profileData || {};
...
function matchesUserFilters(userData, filters, profileData = {}) {
  const f = filters || {};
...
async function collectAudienceRecipients(admin, audienceInput, channel = 'email') {
